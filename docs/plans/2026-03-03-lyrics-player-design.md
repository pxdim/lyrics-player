...
### 10.1 Electron 桌面版 (NDI 輸出)

**目標**: 提供 Windows/Mac 桌面應用，原生輸出 NDI 訊號給 Resolume Arena
...
| 功能 | 說明 | 優先級 |
|------|------|--------|
| NDI 輸出 | 使用免費 NDI SDK 輸出 RGB 視訊 | P0 |
| 房間連接 | 支援連接碼連接現有 Web 顯示端 | P0 |
| 即時同步 | 透過 Supabase Realtime 同步狀態 | P0 |
| 全螢幕模式 | 獨立全螢幕 NDI 輸出 | P1 |
| NDI 名稱設定 | 自訂 NDI 來源名稱 | P2 |
| 多輸出支援 | 同時輸出 NDI + 本地顯示 | P2 |

**技術棧**:
| 層級 | 技術選擇 |
|------|----------|
| 框架 | Electron (latest) |
| NDI SDK | NDI Standard SDK (免費) |
| 視訊擷取 | desktopCapturer API |
| 狀態同步 | Supabase Realtime (與 Web 版共用) |
| 打包 | electron-builder |

**開發工時估計**: 2-3 週

**授權成本**: $0 (使用免費 Standard SDK)

**實作重點**:
1. 使用 `desktopCapturer` 擷取 WebView 內容
2. 整合 NDI SDK 原生模組
3. 保持與 Web 版相同的 Supabase 連線邏輯
4. 提供簡化的控制介面（僅顯示功能）
